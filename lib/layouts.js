...
function formatJson(trace, data, key, cache) {
  ...
  return util.format.apply([trace, JSON.stringify(logData)]);
}

function formatLog(loggingEvent) {
  const data = loggingEvent.data; // [trace, data:{ data }, key]
  const cache = loggingEvent.logger && loggingEvent.logger.cache || {};
  if (Array.isArray(data)) {
    data.push(cache);
    return `${timestampLevelAndCategory(loggingEvent, undefined)}${formatJson.apply(null, data)}`;
  }
  return '';
}
